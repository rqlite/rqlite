...
func TestCreateTableStatement_String(t *testing.T) {
	...
	AssertStatementStringer(t, &sql.CreateTableStatement{
		...
		Columns: []*sql.ColumnDefinition{{
			...
			Constraints: []sql.Constraint{
				&sql.PrimaryKeyConstraint{Asc: pos(0), Autoincrement: pos(0)},
				&sql.PrimaryKeyConstraint{Desc: pos(0)},
				...
				&sql.CollateConstraint{Name: &sql.Ident{Name: "coll"}, CollationName: &sql.Ident{Name: "en_US"}},
			},
		}},
	}, `CREATE TABLE "foo" ("bar" INTEGER PRIMARY KEY ASC AUTOINCREMENT PRIMARY KEY DESC CONSTRAINT "nn" NOT NULL CONSTRAINT "def" DEFAULT 123 DEFAULT (456) UNIQUE CONSTRAINT "coll" COLLATE "en_US")`)
		...
		Columns: []*sql.ColumnDefinition{{
			...
			Constraints: []sql.Constraint{
				&sql.PrimaryKeyConstraint{
					OnConflict: &sql.ConflictClause{Rollback: pos(0)},
				},
			},
		}},
	}, `CREATE TABLE "foo" ("bar" INTEGER PRIMARY KEY ON CONFLICT ROLLBACK)`)
		...
		Columns: []*sql.ColumnDefinition{{
			...
			Constraints: []sql.Constraint{
				&sql.NotNullConstraint{
					OnConflict: &sql.ConflictClause{Abort: pos(0)},
				},
			},
		}},
	}, `CREATE TABLE "foo" ("bar" INTEGER NOT NULL ON CONFLICT ABORT)`)
	...
	AssertStatementStringer(t, &sql.CreateTableStatement{
		...
		Columns: []*sql.ColumnDefinition{{
			...
			Type: &sql.Type{Name: &sql.Ident{Name: "INTEGER"}},
			Constraints: []sql.Constraint{
				&sql.UniqueConstraint{OnConflict: &sql.ConflictClause{Fail: pos(0)}},
				&sql.UniqueConstraint{OnConflict: &sql.ConflictClause{Ignore: pos(0)}},
				&sql.UniqueConstraint{OnConflict: &sql.ConflictClause{Replace: pos(0)}},
			},
		}},
	}, `CREATE TABLE "foo" ("bar" INTEGER UNIQUE ON CONFLICT FAIL UNIQUE ON CONFLICT IGNORE UNIQUE ON CONFLICT REPLACE)`)
...
func TestCollateExpr_String(t *testing.T) {
	AssertExprStringer(t, &sql.CollateExpr{X: &sql.Ident{Name: "foo"}, Name: &sql.Ident{Name: "en_US"}}, `"foo" COLLATE "en_US"`)
}
