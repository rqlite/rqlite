...
func TestParser_ParseStatement(t *testing.T) {
	...
	t.Run("CreateTable", func(t *testing.T) {
		...
		t.Run("ColumnConstraint", func(t *testing.T) {
			t.Run("PrimaryKey", func(t *testing.T) {
				...
				t.Run("Full", func(t *testing.T) {
					stmt := ParseStatementOrFail(t, `CREATE TABLE tbl (col1 TEXT CONSTRAINT cons1 PRIMARY KEY ASC AUTOINCREMENT)`).(*sql.CreateTableStatement)
					if diff := deep.Equal(stmt.Columns[0].Constraints[0], &sql.PrimaryKeyConstraint{
						...
						Key:           pos(53),
						Asc:           pos(57),
						Autoincrement: pos(61),
					}); diff != nil {
						t.Fatal(diff)
					}
				})
				t.Run("Desc", func(t *testing.T) {
					stmt := ParseStatementOrFail(t, `CREATE TABLE tbl (col1 TEXT PRIMARY KEY DESC)`).(*sql.CreateTableStatement)
					cons := stmt.Columns[0].Constraints[0].(*sql.PrimaryKeyConstraint)
					if got, want := cons.Desc, pos(40); got != want {
						t.Fatalf("SortDirection=%s, want %s", got, want)
					}
				})
			...
			t.Run("NotNull", func(t *testing.T) {
				...
				t.Run("OnConflict", func(t *testing.T) {
					stmt := ParseStatementOrFail(t, `CREATE TABLE tbl (col1 TEXT NOT NULL ON CONFLICT ROLLBACK)`).(*sql.CreateTableStatement)
					if diff := deepEqual(stmt.Columns[0].Constraints[0], &sql.NotNullConstraint{
						Not:  pos(28),
						Null: pos(32),
						OnConflict: &sql.ConflictClause{
							On:       pos(37),
							Conflict: pos(40),
							Rollback: pos(49),
						},
				...
				t.Run("ErrOnConflict", func(t *testing.T) {
					AssertParseStatementError(t, `CREATE TABLE tbl (col1 TEXT NOT NULL ON`, `1:39: expected CONFLICT, found 'EOF'`)
				})
			})

			t.Run("Unique", func(t *testing.T) {
				t.Run("ErrInvalidConflict", func(t *testing.T) {
					AssertParseStatementError(t, `CREATE TABLE tbl (col1 INTEGER UNIQUE ON`, `1:40: expected CONFLICT, found 'EOF'`)
				})
				t.Run("Simple", func(t *testing.T) {
				...
				t.Run("OnConflict", func(t *testing.T) {
					stmt := ParseStatementOrFail(t, `CREATE TABLE tbl (col1 TEXT UNIQUE ON CONFLICT ROLLBACK)`).(*sql.CreateTableStatement)
					if diff := deepEqual(stmt.Columns[0].Constraints[0], &sql.UniqueConstraint{
						Unique: pos(28),
						OnConflict: &sql.ConflictClause{
							On:       pos(35),
							Conflict: pos(38),
			...
			t.Run("Collate", func(t *testing.T) {
				AssertParseStatement(t, `CREATE TABLE tbl (col1 TEXT COLLATE coll1)`, &sql.CreateTableStatement{
					Create: pos(0),
					Table:  pos(7),
					Name:   &sql.Ident{Name: "tbl", NamePos: pos(13)},
					Lparen: pos(17),
					Columns: []*sql.ColumnDefinition{
						{
							...
							Constraints: []sql.Constraint{
								&sql.CollateConstraint{
									Collate:       pos(28),
									CollationName: &sql.Ident{Name: "coll1", NamePos: pos(36)},
								},
							},
						},
					},
		...
		t.Run("OnConflict", func(t *testing.T) {
			t.Run("ErrNoConflict", func(t *testing.T) {
				AssertParseStatementError(t, `CREATE TABLE tbl (col1 TEXT PRIMARY KEY ON`, `1:42: expected CONFLICT, found 'EOF'`)
			})
			t.Run("Rollback", func(t *testing.T) {
				stmt := ParseStatementOrFail(t, `CREATE TABLE tbl (col1 TEXT PRIMARY KEY ON CONFLICT ROLLBACK)`).(*sql.CreateTableStatement)
				cons := stmt.Columns[0].Constraints[0].(*sql.PrimaryKeyConstraint)
				if diff := deepEqual(cons.OnConflict, &sql.ConflictClause{
					On:       pos(40),
					Conflict: pos(43),
					Rollback: pos(52),
				}); diff != "" {
			...
			t.Run("Abort", func(t *testing.T) {
				stmt := ParseStatementOrFail(t, `CREATE TABLE tbl (col1 TEXT PRIMARY KEY ON CONFLICT ABORT)`).(*sql.CreateTableStatement)
				cons := stmt.Columns[0].Constraints[0].(*sql.PrimaryKeyConstraint)
				if got, want := cons.OnConflict.Abort, pos(52); got != want {
					t.Fatalf("ResolutionType=%s, want %s", got, want)
				}
			})
			t.Run("Fail", func(t *testing.T) {
				stmt := ParseStatementOrFail(t, `CREATE TABLE tbl (col1 TEXT PRIMARY KEY ON CONFLICT FAIL)`).(*sql.CreateTableStatement)
				cons := stmt.Columns[0].Constraints[0].(*sql.PrimaryKeyConstraint)
				if got, want := cons.OnConflict.Fail, pos(52); got != want {
					t.Fatalf("ResolutionType=%s, want %s", got, want)
				}
			})
			t.Run("Ignore", func(t *testing.T) {
				stmt := ParseStatementOrFail(t, `CREATE TABLE tbl (col1 TEXT PRIMARY KEY ON CONFLICT IGNORE)`).(*sql.CreateTableStatement)
				cons := stmt.Columns[0].Constraints[0].(*sql.PrimaryKeyConstraint)
				if got, want := cons.OnConflict.Ignore, pos(52); got != want {
					t.Fatalf("ResolutionType=%s, want %s", got, want)
				}
			})
			t.Run("Replace", func(t *testing.T) {
				stmt := ParseStatementOrFail(t, `CREATE TABLE tbl (col1 TEXT PRIMARY KEY ON CONFLICT REPLACE)`).(*sql.CreateTableStatement)
				cons := stmt.Columns[0].Constraints[0].(*sql.PrimaryKeyConstraint)
				if got, want := cons.OnConflict.Replace, pos(52); got != want {
					t.Fatalf("ResolutionType=%s, want %s", got, want)
				}
			})
			AssertParseStatementError(t, `CREATE TABLE tbl (col1 TEXT PRIMARY KEY ON CONFLICT foo`, `1:53: expected ROLLBACK, ABORT, FAIL, IGNORE, or REPLACE, found foo`)
		})
	})
...
func TestParser_ParseExpr(t *testing.T) {
	...
	t.Run("Call", func(t *testing.T) {
		...
		AssertParseExpr(t, `sum() over (win1 partition by foo, bar order by baz collate coll1 ASC NULLS FIRST, biz)`, &sql.Call{
			Name:   &sql.Ident{NamePos: pos(0), Name: "sum"},
			Lparen: pos(3),
			Rparen: pos(4),
			Over: &sql.OverClause{
					...
					OrderingTerms: []*sql.OrderingTerm{
						{
							X: &sql.CollateExpr{
								...
								Collate: pos(52),
								Name:    &sql.Ident{NamePos: pos(60), Name: "coll1"},
							},
							Asc:        pos(66),
							Nulls:      pos(70),
							NullsFirst: pos(76),
						...
						{
							X: &sql.Ident{NamePos: pos(83), Name: "biz"},
						},
					},
					Rparen: pos(86),
				},
		...
		AssertParseExprError(t, `sum() over (order by foo collate`, `1:32: expected collation name, found 'EOF'`)
