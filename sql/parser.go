...
func (p *Parser) parsePrimaryKeyConstraint(constraintPos Pos, name *Ident, isTable bool) (_ *PrimaryKeyConstraint, err error) {
	...
	if cons.OnConflict, err = p.parseConflictClause(); err != nil {
		return &cons, err
	}
...
func (p *Parser) parseCollateConstraint(constraintPos Pos, name *Ident) (_ *CollateConstraint, err error) {
	...
	if cons.CollationName, err = p.parseIdent("collation name"); err != nil {
		return &cons, p.errorExpected(p.pos, p.tok, "collation name")
	}
	return &cons, nil
}
...
func (p *Parser) parseConflictClause() (_ *ConflictClause, err error) {
	...
	var cl ConflictClause
	cl.On, _, _ = p.scan()

	if p.peek() != CONFLICT {
		return &cl, p.errorExpected(p.pos, p.tok, "CONFLICT")
	}
	cl.Conflict, _, _ = p.scan()

	switch p.peek() {
	case ROLLBACK:
		cl.Rollback, _, _ = p.scan()
	case ABORT:
		cl.Abort, _, _ = p.scan()
	case FAIL:
		cl.Fail, _, _ = p.scan()
	case IGNORE:
		cl.Ignore, _, _ = p.scan()
	case REPLACE:
		cl.Replace, _, _ = p.scan()
	default:
		return &cl, p.errorExpected(p.pos, p.tok, "ROLLBACK, ABORT, FAIL, IGNORE, or REPLACE")
	}
...
func (p *Parser) parseSource() (source Source, err error) {
	// Parse either a paren-wrapped subsource (e.g. SELECT) or a table name.
	switch p.peek() {
	case LP:
		if source, err = p.parseParenSource(); err != nil {
			return source, err
		}
	case IDENT, QIDENT:
		if source, err = p.parseQualifiedTableName(); err != nil {
			return source, err
		}
	default:
	...
	// Parse join operator.
	join := &JoinClause{X: source}
	if join.Operator, err = p.parseJoinOperator(); err != nil {
		return join, err
	} else if join.Y, err = p.parseSource(); err != nil {
		return join, err
	} else if join.Constraint, err = p.parseJoinConstraint(); err != nil {
		return join, err
	}
...
func (p *Parser) ParseExpr() (expr Expr, err error) {
	...
	case IDENT, QIDENT:
		...
		// Parse "table.column".
		if p.peek() == DOT {
			if expr, err = p.parseQualifiedRef(ident); err != nil {
				return expr, err
			}
		} else if p.peek() == LP {
			if expr, err = p.parseCall(ident); err != nil {
				return expr, err
			}
		} else {
			expr = ident
	...
	case STRING:
		expr = &StringLit{ValuePos: pos, Value: lit}
	case BLOB:
		expr = &BlobLit{ValuePos: pos, Value: lit}
	case FLOAT, INTEGER:
		expr = &NumberLit{ValuePos: pos, Value: lit}
	case NULL:
		expr = &NullLit{Pos: pos}
	case TRUE, FALSE:
		expr = &BoolLit{ValuePos: pos, Value: tok == TRUE}
	case BIND:
		expr = &BindExpr{NamePos: pos, Name: lit}
	case PLUS, MINUS:
		expr, err = p.ParseExpr()
		if err != nil {
			return nil, err
		}
		expr = &UnaryExpr{OpPos: pos, Op: tok, X: expr}
	case LP:
		p.unscan()
		if expr, err = p.parseParenExpr(); err != nil {
			return expr, err
		}
	case CAST:
		p.unscan()
		if expr, err = p.parseCastExpr(); err != nil {
			return expr, err
		}
	case CASE:
		p.unscan()
		if expr, err = p.parseCaseExpr(); err != nil {
			return expr, err
		}
	case RAISE:
		p.unscan()
		if expr, err = p.parseRaise(); err != nil {
			return expr, err
		}
	case NOT, EXISTS:
		p.unscan()
		if expr, err = p.parseExists(); err != nil {
			return expr, err
		}
	default:
	...
	case PLUS, MINUS, STAR, SLASH, REM, CONCAT, NOT, BETWEEN,
		...
		// Read op (and combine multi-word ops).
		var binExpr BinaryExpr
		binExpr.X = expr
		binExpr.OpPos, binExpr.Op, _ = p.scan()
		switch binExpr.Op {
		case IS:
			if p.peek() == NOT {
				p.scan()
		...
		case NOT:
			...
			case REGEXP:
				p.scan()
				binExpr.Op = NOTREGEXP
			case MATCH:
				p.scan()
				binExpr.Op = NOTMATCH
			case BETWEEN:
				p.scan()
				binExpr.Op = NOTBETWEEN
			default:
				return &binExpr, p.errorExpected(p.pos, p.tok, "IN, LIKE, GLOB, REGEXP, MATCH, or BETWEEN")
			}
		}

		// Read RHS.
		if binExpr.Op == IN || binExpr.Op == NOTIN { // IN requires expression list
			if binExpr.Y, err = p.parseExprList(); err != nil {
				return &binExpr, err
			}
		} else if binExpr.Op == BETWEEN || binExpr.Op == NOTBETWEEN { // BETWEEN requires range
			...
			if rng, err := p.ParseExpr(); err != nil {
				return &binExpr, err
			} else if rng, ok := rng.(*BinaryExpr); !ok || rng.Op != AND {
				return &binExpr, p.errorExpected(p.pos, p.tok, "range expression")
			} else {
				binExpr.Y = &Range{X: rng.X, And: rng.OpPos, Y: rng.Y}
			}
		} else {
			if binExpr.Y, err = p.ParseExpr(); err != nil {
				return &binExpr, err
			}
		}
	...
	case COLLATE:
		var collateExpr CollateExpr
		collateExpr.X = expr
		collateExpr.Collate, _, _ = p.scan()
		if collateExpr.Name, err = p.parseIdent("collation name"); err != nil {
			return &collateExpr, err
		}
		return &collateExpr, nil
...
func isConstraintStartToken(tok Token, isTable bool) bool {
	...
	case NOT, DEFAULT, COLLATE, REFERENCES:
