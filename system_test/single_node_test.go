...
func Test_SingleNode(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expected: `{"results":[{"error":"no such table: bar","error_v2":{"code":1,"extended_code":1,"system_errno":0,"message":"no such table: bar"}}]}`,
			execute:  true,
		},
		{
			stmt:     `INSERT blah blah`,
			expected: `{"results":[{"error":"near \"blah\": syntax error","error_v2":{"code":1,"extended_code":1,"system_errno":0,"message":"near \"blah\": syntax error"}}]}`,
			execute:  true,
		},
		{
...
func Test_SingleNode_RETURNING(t *testing.T) {
	...
	// With RETURNING
	res, err = node.Execute(`INSERT INTO foo(id, name) VALUES(6, "denise") RETURNING *`)
	if err != nil {
		t.Fatalf(`write failed: %s`, err.Error())
	}
	if got, exp := res, `{"results":[{"columns":["id","name"],"types":["integer","text"],"values":[[6,"denise"]]}]}`; got != exp {
		t.Fatalf("wrong execute results for RETURNING, exp %s, got %s", exp, got)
	}

	// With RETURNING Associative
	res, err = node.ExecuteAssoc(`INSERT INTO foo(id, name) VALUES(7, "siobhan") RETURNING *`)
	if err != nil {
		t.Fatalf(`write failed: %s`, err.Error())
	}
...
func Test_SingleNodeAutoRestore(t *testing.T) {
	...
	clstr := cluster.New(node.Mux.Listen(cluster.MuxClusterHeader), node.Store, node.Store, mustNewMockCredentialStore())
