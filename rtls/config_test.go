...
func Test_CreateConfig(t *testing.T) {
	...
	config, err := CreateConfig(certFile, keyFile, caCertFile, true, MTLSStateDisabled)
	if err != nil {
	...
	config, err = CreateConfig(certFile, keyFile, caCertFile, false, MTLSStateDisabled)
	if err != nil {
		t.Fatalf("failed to create config: %v", err)
	}
	if config.ClientAuth != tls.NoClientCert {
	...
	config, err = CreateConfig(certFile, keyFile, "", false, MTLSStateEnabled)
	if err != nil {
		t.Fatalf("failed to create config: %v", err)
	}
