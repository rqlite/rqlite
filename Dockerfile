...
RUN cd /extensions/icu && zip icu.zip icu.so && rm icu.so
...
RUN cd /extensions/sqlean && zip sqlean.zip sqlean.so && rm sqlean.so
...
RUN cd /extensions/sqlite-vec && zip sqlite-vec.zip *.so && rm *.so

WORKDIR /app
RUN mkdir -p /extensions/misc
RUN cd extensions/src/misc/ && make && cp *.so /extensions/misc
RUN cd /extensions/misc && zip misc.zip *.so && rm *.so
