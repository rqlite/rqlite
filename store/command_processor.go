...
func (c *CommandProcessor) Process(data []byte, db *sql.SwappableDB) (*proto.Command, bool, any) {
	...
	case proto.Command_COMMAND_TYPE_QUERY:
		...
		r, cdcEvents, err := db.Query(qr.Request, qr.Timings)
		return cmd, false, &fsmQueryResponse{rows: r, error: err, PendingCDCEvents: cdcEvents}
	case proto.Command_COMMAND_TYPE_EXECUTE:
		...
		r, cdcEvents, err := db.Execute(er.Request, er.Timings)
		return cmd, true, &fsmExecuteQueryResponse{results: r, error: err, PendingCDCEvents: cdcEvents}
	case proto.Command_COMMAND_TYPE_EXECUTE_QUERY:
		...
		r, cdcEvents, err := db.Request(eqr.Request, eqr.Timings)
		return cmd, ExecuteQueryResponses(r).Mutation(), &fsmExecuteQueryResponse{results: r, error: err, PendingCDCEvents: cdcEvents}
	case proto.Command_COMMAND_TYPE_LOAD:
		...
		if err != nil {
			return cmd, false, &fsmGenericResponse{error: fmt.Errorf("failed to create temporary database file: %s", err), PendingCDCEvents: nil}
		}
		defer os.Remove(fd.Name())
		defer fd.Close()
		_, err = fd.Write(lr.Data)
		if err != nil {
			return cmd, false, &fsmGenericResponse{error: fmt.Errorf("failed to write to temporary database file: %s", err), PendingCDCEvents: nil}
		}
		fd.Close()

		// Swap the underlying database to the new one.
		if err := db.Swap(fd.Name(), db.FKEnabled(), db.WALEnabled()); err != nil {
			return cmd, false, &fsmGenericResponse{error: fmt.Errorf("error swapping databases: %s", err), PendingCDCEvents: nil}
		}
		// LOAD operations replace the DB, so any prior CDC events from the old DB instance are irrelevant.
		return cmd, true, &fsmGenericResponse{PendingCDCEvents: nil}
	case proto.Command_COMMAND_TYPE_LOAD_CHUNK:
		...
		if err != nil {
			return cmd, false, &fsmGenericResponse{error: fmt.Errorf("failed to get dechunker: %s", err), PendingCDCEvents: nil}
		}
		if lcr.Abort {
			path, err := dec.Close()
			if err != nil {
				return cmd, false, &fsmGenericResponse{error: fmt.Errorf("failed to close dechunker: %s", err), PendingCDCEvents: nil}
			}
			...
			if err != nil {
				return cmd, false, &fsmGenericResponse{error: fmt.Errorf("failed to write chunk: %s", err), PendingCDCEvents: nil}
			}
			if last {
				path, err := dec.Close()
				if err != nil {
				...
				if !sql.IsValidSQLiteFile(path) {
					...
					return cmd, false, &fsmGenericResponse{error: fmt.Errorf("invalid chunked database file - ignoring"), PendingCDCEvents: nil}
				}
		...
		// LOAD_CHUNK operations also effectively replace the DB or part of it, CDC events from previous DB state are not relevant.
		return cmd, true, &fsmGenericResponse{PendingCDCEvents: nil}
	case proto.Command_COMMAND_TYPE_NOOP:
		return cmd, false, &fsmGenericResponse{PendingCDCEvents: nil}
	default:
		return cmd, false, &fsmGenericResponse{error: fmt.Errorf("unhandled command: %v", cmd.Type), PendingCDCEvents: nil}
	}
}
