...
func Test_SingleNodeBackupBinary_NoLogs(t *testing.T) {
	...
	defer f.Close()
	if _, err := s.Backup(context.Background(), backupRequestBinary(true, false, false), f); err != nil {
...
`
	...
	defer gzf.Close()
	if _, err := s.Backup(context.Background(), backupRequestBinary(true, false, true), gzf); err != nil {
...
func Test_StoreSingleNodeNotOpen(t *testing.T) {
	...
	if _, _, err := s.Execute(context.Background(), &proto.ExecuteRequest{}); err != ErrNotOpen {
		t.Fatalf("wrong error received for non-open store: %s", err)
	}
	if _, _, _, err := s.Query(context.Background(), &proto.QueryRequest{}); err != ErrNotOpen {
		t.Fatalf("wrong error received for non-open store: %s", err)
	}
	if err := s.Load(context.Background(), nil); err != ErrNotOpen {
		t.Fatalf("wrong error received for non-open store: %s", err)
	}
}
...
func Test_SingleNodeExecuteQuery_EXPLAIN(t *testing.T) {
	...
	qr := queryRequestFromString("EXPLAIN QUERY PLAN SELECT * FROM foo", false, false, false)
	qr.Level = proto.ConsistencyLevel_WEAK
	...
	qr = queryRequestFromString(`EXPLAIN QUERY PLAN INSERT INTO foo(name) VALUES("fiona")`, false, false, false)
	qr.Level = proto.ConsistencyLevel_WEAK
	qr.Request.Statements[0].SqlExplain = true
	rows, _, _, err = s.Query(context.Background(), qr)
	if err != nil {
...
func Test_SingleNodeExecuteQuery_RETURNING(t *testing.T) {
	...
	rows, _, err := s.Execute(context.Background(), er)
	if err != nil {
...
func Test_SingleNodeExecuteQueryTx(t *testing.T) {
	...
	qr := queryRequestFromString("SELECT * FROM foo", false, true, false)
...
func Test_SingleNodeRequestParameters(t *testing.T) {
	...
	for _, tt := range tests {
		r, _, _, err := s.Request(context.Background(), tt.request)
		if err != nil {
			t.Fatalf("failed to execute request on single node: %s", err.Error())
		}
...
func Test_SingleNodeOnDiskFileExecuteQuery(t *testing.T) {
	...
	qr = queryRequestFromString("SELECT * FROM foo", true, false, false)
...
func Test_SingleNodeExecuteQueryFreshness(t *testing.T) {
	...
	_, idx, err := s0.Execute(context.Background(), er)
	if err != nil {
		t.Fatalf("failed to execute on single node: %s", err.Error())
	}
	testPoll(t, func() bool {
...
`
	...
	if _, err := s.Backup(context.Background(), backupRequestSQL(true), f); err != nil {
		t.Fatalf("Backup failed %s", err.Error())
	}
...
`
	...
	if _, err := s.Backup(context.Background(), backupRequestDelete(true, false, false), f); err != nil {
	...
	if _, err := s.Backup(context.Background(), backupRequestDelete(true, false, true), gzf); err != nil {
...
func Test_SingleNodeLoadTextChinook(t *testing.T) {
	...
	_, _, err := s.Execute(context.Background(), executeRequestFromString(chinook.DB, false, false))
	if err != nil {
	...
	qr := queryRequestFromString("SELECT count(*) FROM track", false, true, false)
	qr.Level = proto.ConsistencyLevel_STRONG
	r, _, _, err := s.Query(context.Background(), qr)
	if err != nil {
		t.Fatalf("failed to query single node: %s", err.Error())
	...
	qr = queryRequestFromString("SELECT count(*) FROM album", false, true, false)
	qr.Level = proto.ConsistencyLevel_STRONG
	r, _, _, err = s.Query(context.Background(), qr)
	if err != nil {
		t.Fatalf("failed to query single node: %s", err.Error())
	...
	qr = queryRequestFromString("SELECT count(*) FROM artist", false, true, false)
	qr.Level = proto.ConsistencyLevel_STRONG
	r, _, _, err = s.Query(context.Background(), qr)
	if err != nil {
		t.Fatalf("failed to query single node: %s", err.Error())
...
`
	...
	// Check that data were loaded correctly.
	qr := queryRequestFromString("SELECT * FROM bar", false, true, false)
	qr.Level = proto.ConsistencyLevel_STRONG
	r, _, _, err := s.Query(context.Background(), qr)
	if err != nil {
		t.Fatalf("failed to query single node: %s", err.Error())
...
`
	...
	rer, _, err := s.Execute(context.Background(), er)
	if err != nil {
...
func Test_StoreExecuteRaftIndex(t *testing.T) {
	...
	_, raftIndex, err := s.Execute(context.Background(), er)
	if err != nil {
		t.Fatalf("failed to execute on single node: %s", err.Error())
	}
	...
	_, raftIndex2, err2 := s.Execute(context.Background(), er2)
	if err2 != nil {
...
func Test_StoreQueryRaftIndex(t *testing.T) {
	...
	_, writeIndex, err := s.Execute(context.Background(), er)
	if err != nil {
		t.Fatalf("failed to execute write request: %s", err.Error())
	}
	if writeIndex == 0 {
	...
	strongReq := queryRequestFromString("SELECT * FROM foo", false, false, false)
	strongReq.Level = proto.ConsistencyLevel_STRONG
	_, _, _, err = s.Query(context.Background(), strongReq)
	if err != nil {
	...
	for _, level := range levels {
		req := queryRequestFromString("SELECT * FROM foo", false, false, false)
		req.Level = level.level
		_, _, index, err := s.Query(context.Background(), req)
		if err != nil {
	...
	strongTestReq := queryRequestFromString("SELECT * FROM foo", false, false, false)
	strongTestReq.Level = proto.ConsistencyLevel_STRONG
	_, _, strongIndex, err := s.Query(context.Background(), strongTestReq)
	if err != nil {
...
func Test_StoreQueryReturnedLevel(t *testing.T) {
	...
	_, level, _, err := s.Query(context.Background(), req)
	if err != nil {
...
func queryRequestFromString(s string, timings, tx, qualify bool) *proto.QueryRequest {
	return queryRequestFromStrings([]string{s}, timings, tx, qualify)
}

// queryRequestFromStrings converts a slice of strings into a proto.QueryRequest
func queryRequestFromStrings(s []string, timings, tx, qualify bool) *proto.QueryRequest {
	stmts := make([]*proto.Statement, len(s))
	for i := range s {
		stmts[i] = &proto.Statement{
			Sql: s[i],
