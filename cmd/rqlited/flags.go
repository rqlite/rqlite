...
func ParseFlags(name, desc string, build *BuildInfo) (*Config, error) {
	...
	flag.DurationVar(&config.RaftReapNodeTimeout, "raft-reap-node-timeout", 48*time.Hour, "Time after which a nonreachable voting node will be reaped")
	flag.DurationVar(&config.RaftReapReadOnlyNodeTimeout, "raft-reap-read-only-node-timeout", 48*time.Hour, "Time after which a non-reachable non-voting node will be reaped")
