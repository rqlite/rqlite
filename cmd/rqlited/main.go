...
func init() {
	...
	flag.StringVar(&sourceIp, "source-ip", "", "Specify a source ip address, when your node has multiple ip address segments")
...
func main() {
	...
	if len(joins) > 0 && isNew {
		...
		if j, err := cluster.Join(sourceIp, joins, str.ID(), advAddr, !raftNonVoter, meta,
