...
func Forge(arguments []string) (*flag.FlagSet, *Config, error) {
	...
	fs.StringVar(&config.HTTPVerifyCommonName, "http-verify-common-name", "", "Required Common Name on client certificates. If not set, any client cert signed by the CA is accepted")
	...
	fs.StringVar(&config.NodeVerifyCommonName, "node-verify-common-name", "", "Required Common Name on incoming peer certificates. If not set, any peer cert signed by the CA is accepted")
