...
type Config struct {
	...
	// Path to Change Data Capture (CDC) configuration file. If not set, not enabled
	CDCFile string
...
func Forge(arguments []string) (*flag.FlagSet, *Config, error) {
	...
	fs.StringVar(&config.CDCFile, "cdc", "", "Path to Change Data Capture (CDC) configuration file. If not set, not enabled")
