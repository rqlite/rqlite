...
func (m *MockVisitor) CalcCRC32(dataPath, crcPath string) error {
	m.Calls = append(m.Calls, "calc_crc32 "+dataPath+"->"+crcPath)
	return m.Err
}

func (m *MockVisitor) VerifyDB(dataPath string) error {
	m.Calls = append(m.Calls, "verify_db "+dataPath)
	return m.Err
}
...
func (f *FailVisitor) VerifyDB(dataPath string) error                   { return f.check() }
