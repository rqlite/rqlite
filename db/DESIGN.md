...
`SwappableDB` is a thin synchronization wrapper around `*DB`. Every method takes an `RWMutex` read lock, delegates to the inner `*DB`, and unlocks. The single method that takes the write lock is `Swap`, which closes the current database, stashes its on-disk files, renames a replacement file into place, reopens the database, and recreates the `CheckpointManager` (which has to start over because the new database has fresh WAL salt values). The stash is removed after the replacement is installed. On failure, Swap attempts to restore and reopen the original database and reports any rollback errors as well. This does not guarantee recovery from arbitrary I/O failures or preserve connection-local hooks, so callers must retain their existing failure handling. The Store handles startup using its clean-snapshot and Raft recovery decisions; file existence alone does not identify a completed swap.
