...
`CDCStreamer` (in `cdc.go`) is the in-package hook target. The Store calls `Reset` with the Raft log index before processing a request. The preupdate hook appends an event to a pending transaction; the commit hook looks up column names per table (cached for that transaction) and accumulates its events in a separate committed group. After processing the request, the Store calls `Flush` to send that group, stamped with the time of its last commit. A request can contain multiple autocommit transactions, but they must share one group: CDC consumers acknowledge Raft indices, so sending separate groups at the same index could let an acknowledgement discard later changes. If the channel is full the group is dropped and a stat increments. The streamer does not back-pressure the writer.
