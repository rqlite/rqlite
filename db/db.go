...
func init() {
	sql.Register("rqlite3", &sqlite3.SQLiteDriver{
		ConnectHook: func(conn *sqlite3.SQLiteConn) error {
			if err := conn.DBConfigNoCkptOnClose(); err != nil {
				return fmt.Errorf("failed to disable checkpoint-on-close: %w", err)
			}
			return nil
		},
	})
...
func Open(dbPath string, fkEnabled, wal bool) (retDB *DB, retErr error) {
	...
	rwDB, err := sql.Open("rqlite3", rwDSN)
	if err != nil {
	...
	roDB, err := sql.Open("rqlite3", roDSN)
	if err != nil {
		return nil, err
	}
