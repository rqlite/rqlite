## 7.9.2 (unreleased)
This release addresses a shortcoming in inter-node communications, ensuring that Protobufs up to 2^64 bytes in size can be transmitted correctly. **If any node in a cluster is upgraded to this release, then all nodes in that cluster should be**. However this change only affects transparently-forwarded queries, loads, and restores, so a rolling upgrade is an option (as long as there is no such traffic during the upgrade -- if there is, clients may receive an error during the upgrade process).
