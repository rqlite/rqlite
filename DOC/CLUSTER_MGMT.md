...
Note that if a node's network addresses change on a restart, that node **must explicitly rejoin the cluster**. Rejoining the cluster is how a node correctly informs the cluster that it has new network addresses.
